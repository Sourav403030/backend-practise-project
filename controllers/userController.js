...
const updateAccountDetails = async (req, res) => {
  try {
    //Get the fullname, email, and username from the request body
...
const getCurrentUser = async (req, res) => {
  ...
  return res.status(200).json({
    ...
    user: req.user, // Return the current user from the request
  });
};
...
const updateUserAvatar = async(req, res) =>{
    try {
        ...
        await userModel.findByIdAndUpdate(req.user?._id, {$set: {avatar: avatar.url}}, {new: true})// Update the user's avatar URL

        return res.status(200).json({
            success: true,
            message: "User avatar updated successfully",
            avatar: avatar.url, // Return the updated avatar URL
        })
